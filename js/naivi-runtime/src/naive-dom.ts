...
import type { EventType } from "./wasm-types.js";
...
export async function loadCSSClassStyles(): Promise<void> {
  // Desktop: the native host injects the compiled styles as a JSON object on
  // globalThis.__NAIVE_STYLES before evaling the bundle (plan 043, U5).
  const injected = (globalThis as unknown as Record<string, unknown>).__NAIVE_STYLES;
  if (injected && typeof injected === "object") {
    const styles = injected as Record<string, unknown> & {
      rules?: unknown[];
    };
    if (isWasmReady() && Array.isArray(styles.rules) && styles.rules.length > 0) {
      // Rust RuleTable::from_json expects the `{ "rules": [...] }` wire
      // shape (plan 060), not the bare array.
      nativeSetRuleTable(JSON.stringify({ rules: styles.rules }));
    }
    return;
  }
  try {
    // Generated by the CLI (`naive wasm`); absent in a fresh checkout, so
    // tsc cannot resolve the virtual absolute path.
    // @ts-expect-error generated artifact may not exist yet
    const mod = await import('/node_modules/.naive/styles.json');
    const styles = (mod.default ?? mod) as Record<string, unknown> & {
      rules?: unknown[];
    };
    if (isWasmReady() && Array.isArray(styles.rules) && styles.rules.length > 0) {
      nativeSetRuleTable(JSON.stringify({ rules: styles.rules }));
    ...
    /* styles.json absent in a fresh checkout */
  }
}
...
function upgradeSubtreeToWasm(node: NaiveNode): void {
  if (node.nodeType === 3) {
    ...
    // Copy attributes already set on the node
    for (const [k, v] of Object.entries(el._attrs)) {
      nativeSetProp(newMirror, k, String(v));
    }
    const cls = el._attrs['class'] || el._attrs['className'] || '';
    if (cls) {
      nativeSetProp(newMirror, 'class', cls);
    }
...
function createStyleStub(el: NaiveElement): CSSStyleDeclaration {
  ...
  // Only forward display (layout) and visibility to WASM — everything else
  // stays client-side to avoid memory pressure.
  const FORWARD_PROPS = new Set(['display', 'visibility']);
  ...
  const proxy = new Proxy(store, {
    ...
    set(_t, key: string, value: string) {
      ...
      store[key] = value;
      // Only forward specific properties to avoid memory pressure
      forward(key, value);
      return true;
    },
  }) as unknown as CSSStyleDeclaration;
  (proxy as any)._mirror = el._mirror;
  return proxy;
}
