...
use crate::{
    ...
    TextNodeData, events::implicit_form_submission, qual_name,
};
use blitz_traits::devtools::DevtoolSettings;
use blitz_traits::events::{
    BlitzInputEvent, BlitzImeEvent, BlitzKeyEvent, BlitzScrollEvent, DomEvent, DomEventData,
    HitResult, KeyState, UiEvent,
};
...
pub struct BaseDocument {
    ...
    /// Active native-edit session (R1–R8): a focused text input/textarea being
    /// edited by a host platform control. `None` when the parley path is in
    /// use. Single-session per document (one focused element).
    pub native_edit_session: Option<NativeEditSession>,
    /// DOM events generated by the native-edit session that must reach the
    /// guest (input / key / composition), drained by the host document's poll
    /// and dispatched handler-only (no default actions, so parley stays
    /// untouched). KTD8/KTD9.
    pub native_edit_pending_events: VecDeque<DomEvent>,
    /// True while the engine mirrors a native-control change back into the
...
/// is owned by a host platform control instead of the canvas parley editor.
#[derive(Debug, Clone)]
pub struct NativeEditSession {
    /// The focused element being natively edited.
    pub node_id: NodeId,
    /// The element's box geometry in viewport coordinates (R10).
    pub geometry: NativeEditGeometry,
    /// Computed style payload for deep style match (R12).
    pub style: NativeEditStyle,
    /// Editing attributes reflected onto the control (R11).
    pub attrs: NativeEditAttrs,
    /// Last value mirrored into the DOM, used to skip no-op `input` dispatch.
...
fn css_generic_family(generic: style::values::computed::font::GenericFontFamily) -> String {
    use style::values::computed::font::GenericFontFamily;
    let keyword = match generic {
        GenericFontFamily::None | GenericFontFamily::SansSerif => "sans-serif",
        GenericFontFamily::Serif => "serif",
        GenericFontFamily::Monospace => "monospace",
        GenericFontFamily::Cursive => "cursive",
        GenericFontFamily::Fantasy => "fantasy",
        GenericFontFamily::SystemUi => "-apple-system, BlinkMacSystemFont, sans-serif",
    };
    keyword.to_string()
}
...
impl BaseDocument {
    ...
    ///
    /// The engine owns the session (KTD1): value changes are mirrored into the
    /// DOM (value attribute + parley editor text) and dispatched to the guest
    /// as `input` events; keys and composition are forwarded to the guest
    /// handler-only (KTD8/KTD9); Tab/Enter route to existing focus traversal /
    ...
    /// attribute + parley editor text, dispatch an `input` event to the guest,
    /// and (for a commit) end the session. Skips no-op dispatches.
    fn native_edit_value_changed(&mut self, node_id: NodeId, value: String, end_session: bool) {
        let is_unchanged = self
            .native_edit_session
            .as_ref()
            .is_some_and(|s| s.last_value == value);
        if !is_unchanged {
            ...
            drop(mutator);
            self.native_edit_mirror_in_progress = false;

            if let Some(session) = self.native_edit_session.as_mut() {
                session.last_value = value.clone();
            }
            self.native_edit_pending_events.push_back(DomEvent::new(
                node_id,
    ...
    /// End the active native-edit session (idempotent). Hides/destroys the
    /// control via the shell provider (R3).
    pub fn end_native_edit_session(&mut self) {
    ...
    /// Begin a native-edit session for `node_id` when it is a covered text
    /// input and the shell hosts a native backend. Fail-closes to the parley
    /// path when the backend cannot create a control.
    fn start_native_edit_session(&mut self, node_id: NodeId) {
        if self.native_edit_session.is_some() {
            return;
        }
        let Some(geometry) = self.native_edit_geometry(node_id) else {
            return;
        };
        let Some(style) = self.native_edit_style(node_id) else {
            return;
        };
        let Some(attrs) = self.native_edit_attrs(node_id) else {
            return;
        };
        let value = self
            .get_node(node_id)
            .and_then(|n| n.element_data())
            .and_then(|e| e.attr(local_name!("value")))
            .unwrap_or("")
            .to_string();
        let ok = self
            .shell_provider
    ...
    fn native_edit_geometry(&self, node_id: NodeId) -> Option<NativeEditGeometry> {
        ...
        Some(NativeEditGeometry {
            content_box: (
                origin.x + layout.content_box_x(),
                origin.y + layout.content_box_y(),
                layout.content_box_width(),
                layout.content_box_height(),
            ),
            border_box: (origin.x, origin.y, layout.size.width, layout.size.height),
        })
    }

    /// Computed style payload for the native control (R12).
    fn native_edit_style(&self, node_id: NodeId) -> Option<NativeEditStyle> {
        ...
        let mut style = NativeEditStyle::default();

        let font = styles.get_font();
        let mut families: Vec<String> = Vec::new();
        for family in font.font_family.families.iter() {
            let name = match family {
                SingleFontFamily::FamilyName(n) => n.name.as_ref().to_string(),
                SingleFontFamily::Generic(g) => css_generic_family(*g),
            };
            if !families.contains(&name) {
                families.push(name);
            }
        }
        style.font_family = families.join(", ");
        style.font_size = font.font_size.used_size.0.px();
        style.font_weight = font.font_weight.value() as u16;

        let current_color = styles.clone_color();
        style.color = srgb_rgba(&current_color);
        style.background_color =
            srgb_rgba(&styles.get_background().background_color.resolve_to_absolute(&current_color));

        let border = styles.get_border();
        style.border_width = layout
            .border
            .top
            .max(layout.border.bottom)
            .max(layout.border.left)
            .max(layout.border.right);
        style.border_color = srgb_rgba(&border.border_top_color.resolve_to_absolute(&current_color));
        style.padding = (
            layout.padding.top,
        ...
        style.text_align = match styles.clone_text_align() {
            TextAlignKeyword::Right => "right".to_string(),
            TextAlignKeyword::Center | TextAlignKeyword::MozCenter => "center".to_string(),
            TextAlignKeyword::Justify => "justify".to_string(),
            _ => "left".to_string(),
        };
    ...
    /// Editing attributes of `node_id` reflected onto the native control (R11).
    fn native_edit_attrs(&self, node_id: NodeId) -> Option<NativeEditAttrs> {
        let node = self.get_node(node_id)?;
        let element = node.element_data()?;
        let multiline = element
            .text_input_data()
            .is_some_and(|data| data.is_multiline);
        Some(NativeEditAttrs {
            placeholder: element
                .attr(local_name!("placeholder"))
                .unwrap_or("")
                .to_string(),
            max_length: element.attr_parsed(local_name!("maxlength")),
            read_only: element.has_attr(local_name!("readonly")),
    ...
    /// Take the DOM events queued by the session for guest dispatch (drained
    /// by the host document's poll, handler-only).
    pub fn drain_native_edit_events(&mut self) -> Vec<DomEvent> {
        self.native_edit_pending_events.drain(..).collect()
    }
    ...
    pub(crate) fn clear_interaction_state_for_removed_node(&mut self, node_id: NodeId) {
        ...
        // A removed focused input ends its native-edit session (R3 teardown).
        if self
            .native_edit_session
            .as_ref()
    ...
    pub fn clear_focus(&mut self) {
        ...
        // A native-edit session ends with its focus. The value is already
        // live-mirrored keystroke-by-keystroke (R4), so no extra commit is
        // needed here; a backend `Committed` that arrives afterwards is
        // dropped by the stale-event guard.
        self.end_native_edit_session();
    }
    ...
    pub fn set_focus_to(&mut self, focus_node_id: NodeId) -> bool {
        ...
        let native_edit = shell_provider.native_edit_capable()
            && self
        ...
        // End any previous session, then start one for the newly focused
        // covered input (fail-closing to the parley path when unsupported).
        self.end_native_edit_session();
        if native_edit {
