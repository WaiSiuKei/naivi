...
- **Objective:** 把 naivi 的 JS↔Rust 桥接协议升级为「TS 单一事实来源 + 二进制帧」：新增独立 `js/naivi-protocol` 包，用单一字面量表定义事件 kind / op / 事件 payload 槽位；Rust 侧由 `naivi-dom` 的 build.rs 在构建时自动生成枚举与常量，漂移在构造上不可能；在此之上把 wasm 与 native 双通道从逐 op 直调改为每帧一批的二进制帧传输，节点 id 切到 JS 虚拟 id + Rust 映射，事件回流同样走帧。一个 plan、两个阶段：先 SOT 打底、再帧批处理。
...
**Product Contract preservation:** changed — 新增 R14/R15（帧拒绝预防 + 自愈）与 F3/AE7，KD4 修订，新增 KD8；其余 R/A/F/AE/KD 保持原样。

### Summary

naivi 的桥接协议从「TS 与 Rust 两侧手写、靠约定同步」升级为「TS 单一事实来源 + build.rs 构建期生成」，并在此基础上把双通道从逐 op 直调改为每帧一批的二进制帧传输（id 切到 JS 虚拟 id + Rust 映射、事件回流同样走帧）。两阶段交付：阶段一 SOT 打底独立生效，阶段二帧批处理在其上落地。
...
- R1. 协议定义收敛到独立 `js/naivi-protocol` 包内的单一字面量表，包含：事件 kind 表（12 种 wire kind，各带稳定字符串名与 u8 号）、op 表（帧内编号）、事件 payload 槽位表；`change` 作为合成事件标记（见 R10），不进入 wire 表。
- R2. SOT 表必须是裸字面量（`const X = { … } as const`）：build.rs 用朴素解析读取、不执行 TS；JS 侧正常 `import` 同一份表。表格式变更需要同步更新解析器。
- R3. Rust 侧由 `naivi-dom` 的 build.rs 在 cargo build 时读取 SOT 表，生成 `NaiviEventKind` 完整枚举与 `to_u8` / `from_u8` / `from_str` / `name` / `ALL`、op 常量、payload 槽位常量；生成物写 `OUT_DIR`、不提交；SOT 文件变更通过 `cargo:rerun-if-changed` 触发重生成。
...
- R8. 事件回流同样走二进制帧：按事件 kind 与 payload 槽位表编码，含变长字符串 `key` / `code` / `value`。
...
flowchart LR
  ...
  B --> G["OUT_DIR 生成物<br/>NaiviEventKind + op/payload 常量"]
...
- KD2. 事件回流也走二进制帧。(session-settled: user-directed — chosen over 逐条函数回调: 对称性与吞吐优先，代价是协议需带变长字符串 payload。Governs R8、R10)
...
- F1. 帧生命周期（阶段二核心行为）
  ...
  - **Steps:** renderer 把 op 压入 writer 缓冲 → 帧边界（rAF / vsync tick）一次 flush `Uint8Array` → Rust 完整解码 + 校验 → 整帧 apply（虚拟 id → blitz id 映射建树 / 改样式 / 绑事件）→ 布局渲染 → 命中测试得 blitz 节点 → 反查虚拟 id → 编码事件帧回传 JS → JS dispatch 到监听器。
  - **Outcome:** 每个方向每帧一次边界穿越；失败帧整帧丢弃（Covers R6、R7、R8、R9）。
...
- AE6. 事件帧（Covers R8、R10）：checkbox 切换在 wasm 与 native 上触发合成 `change`；`key` / `code` / `value` 随事件帧正确到达 JS。
...
- OQ3. 事件帧 payload 布局与 op 编号 —— 已解决（KTD1/KTD6，U4 落地）；`frame_rejected` 载体（独立回调 vs 事件 kind）实施期二选一，倾向独立回调（Deferred to Implementation）。
...
- KTD1. 帧线格式（参考 blitz-quick 线格式）。（Governs R6、R8）
  - DOM 变更帧（JS→Rust）：`[seq: u32][count: u16][op…]`；每个 op `[opcode: u8][operands]`；字符串 `[len: u16][utf8]`。
  - 事件帧（Rust→JS）：`[count: u16][event…]`；每个事件 `[虚拟id: u32][kind: u8][槽位…]`，字符串同 u16 前缀。
  - op 表编号、事件 payload 槽位顺序由 SOT 表定义（U4 落地）；`reset` 为保留 op。
- KTD2. JS 虚拟 id 模型。（Governs R7）
  ...
  - 事件帧携带虚拟 id；JS 镜像与 `_elByWasmId` 键仍按 id 语义工作（id 值从 blitz id 变为虚拟 id）。
...
- KTD4. build.rs 生成机制。（Governs R3）
  - `packages/naivi-dom/build.rs` 朴素正则解析 SOT 裸字面量表，输出 `OUT_DIR/protocol_gen.rs`（`NaiviEventKind` 枚举 + `to_u8`/`from_u8`/`from_str`/`name`/`ALL` + op/payload 常量），src 经 include! 接入。
...
- KTD6. 事件帧与拒绝信号编码。（Governs R8）
  - 事件帧按 KTD1；`frame_rejected` 作为独立 Rust→JS 回调（`set_frame_rejected_callback`）而非事件 kind——它不是 DOM 事件；实施期若发现并入事件帧更简，可二选一（OQ3）。
...
flowchart LR
  ...
  H -->|"读 data-naivi-id 反查"| R2["事件帧带虚拟 id"]
  R2 -->|"dispatch"| JS
```
...
- **`frame_rejected` 载体**：独立回调 vs 事件帧保留 kind。倾向独立回调（KTD6），实施期可调。
...
#### U4. 帧协议：op 表 + payload 槽位 + 生成 + TS 编解码助手
- **Goal:** 扩展 SOT 为完整帧协议（op 表编号、事件 payload 槽位表、`reset` op、`frame_rejected` 信号），build.rs 生成 op/payload 常量；TS 侧提供帧 Writer/Reader 助手与帧格式单测。
- **Requirements:** R1、R6、R8、R10、R14
- **Dependencies:** U1、U2、U3
- **Files:**
  - `js/naivi-protocol/src/index.ts`（修改：加 op 表 + payload 槽位表 + `reset` 标记 + `frame_rejected` 信号）
  - `js/naivi-protocol/src/writer.ts`（新建：帧 Writer，Uint8Array 累积、u16 长度前缀、op 发射、取帧）
  - `js/naivi-protocol/src/reader.ts`（新建：帧 Reader 与事件帧解码，供 JS 事件分发）
  - `packages/naivi-dom/build.rs`（修改：生成 op/payload 常量）
  - `js/naivi-protocol/tests/frame-format.test.ts`（新建）
- **Approach:**
  1. SOT 增加：op 表（编号，含 create/attr/style/append/insert/remove/bind/reset 等）、事件 payload 槽位表（坐标/key/code/value 等）、`frame_rejected` 信号定义（KTD1）。
  2. TS Writer：小端、u16 前缀字符串、opcode u8；超 u16 上限抛错而非产出坏帧（R14）。
  3. TS Reader：解码事件帧（虚拟 id u32 + kind u8 + 槽位）供 JS dispatch。
  4. 帧头 `[seq u32][count u16]`（DOM 变更帧）与 `[count u16]`（事件帧）按 KTD1。
- **Patterns to follow:** blitz-quick `protocol.rs` 的线格式与 writer 思路；本包裸字面量表风格。
- **Test scenarios:**
  - Writer 编码 → Reader 解码往返一致（所有 op 类型）。
  - 帧头 seq/count 正确；空 writer flush 输出空帧或跳过。
  - 超 u16 字符串：writer 抛错、不产出坏帧（Covers R14）。
  - 事件帧解码：多事件、变长字符串 key/code/value、未知 kind 兜底。
  - op/payload 常量生成与 SOT 一致。
- **Verification:** `pnpm -r typecheck && pnpm -r test` 绿；`cargo build -p naivi-dom` 后 OUT_DIR 含 op/payload 常量。

#### U5. JS 桥接层重写：writer + 虚拟 id + flush + 事件帧分发 + 自愈触发
- **Goal:** 把 runtime 的逐 op 宿主调用改为「写 writer + 帧边界 flush」；引入 JS 虚拟 id 分配/回收；事件帧解码分发；`frame_rejected` → `reset` → 全量重挂载恢复路径。
- **Requirements:** R6、R7、R8、R9、R10、R14、R15
- **Dependencies:** U4
- **Files:**
  - `js/naivi-runtime/src/batched-bridge.ts`（修改：透传改为真 writer 接入 + flush）
  - `js/naivi-runtime/src/native-tree.ts`（修改：per-op 调用点改 writer；虚拟 id 分配/回收；事件帧解码分发；`frame_rejected` 注册）
...
- **Approach:**
  ...
  3. 事件分发：`set_event_callback` 收到的字节 → Reader 解码 → 按虚拟 id dispatch（原 `dispatchHostEvent` 逻辑保留，输入变虚拟 id）。
...
- **Test scenarios:**
  ...
  - 事件帧解码 → dispatch → `change` 合成（checkbox）与键盘/输入（key/code/value）不变（Covers AE6）。
...
- **Execution note:** 帧格式以 U4 测试向量为准；先写解码往返测试再实现 writer/reader。

#### U6. Rust 帧核心：decode/apply/事务/reset/事件编码/拒绝回传
- **Goal:** 在 `naivi-dom` 实现引擎中立的帧解码、整帧事务 apply（虚拟 id 映射）、`reset`、事件帧编码与 `frame_rejected` 队列；`NaiviOp` create 变体加虚拟 id。
- **Requirements:** R6、R7、R8、R9、R10、R15
- **Dependencies:** U4
- **Files:**
  - `packages/naivi-dom/src/frame.rs`（新建：FrameDecoder / FrameApplier / 事件帧编码 / 校验）
  ...
  - `packages/naivi-dom/src/ffi.rs`（修改：`flush_frame`/`set_frame_rejected_callback` 入口 + 事件帧编码调用点）
...
- **Approach:**
  ...
  5. 事件帧编码：命中 → 读 `data-naivi-id` → 编码（虚拟 id + kind + 槽位 + key/code/value 字符串）。
...
- **Test scenarios:**
  ...
  - 事件帧编码：命中节点 → 带虚拟 id 与字符串 payload 的事件帧正确。
...
#### U7. wasm 通道切换
- **Goal:** wasm host 从 per-op 导出改为 `flush_frame` + 事件帧回传 + `frame_rejected` 回调；删 per-op wasm-bindgen 导出。
- **Requirements:** R6、R7、R8、R10、R11
- **Dependencies:** U6
- **Files:**
  - `packages/naivi-wasm/src/lib.rs`（修改：导出 `flush_frame`、`set_frame_rejected_callback`；`WasmEventSink` 改事件帧编码；删 per-op 导出；tick 机制调整）
  - `packages/naivi-wasm/tests/ops_surface.rs`（修改：跟随新导出面）
- **Approach:**
  1. `flush_frame` 作为唯一 DOM 变更入口，内部走 U6 的 decoder/applier。
  2. `WasmEventSink` 把事件编码成事件帧字节回传 JS 回调。
...
- **Test scenarios:**
  ...
  - 事件帧：合成 pointer 事件 → 回调收到含虚拟 id 的事件帧。
...
#### U8. native 通道切换
- **Goal:** rquickjs FFI 从 per-op 命名空间改为 `flush_frame` + 事件帧 + `frame_rejected`；guest pump/drain 更新。
- **Requirements:** R6、R7、R8、R10、R11
- **Dependencies:** U6
- **Files:**
  - `packages/naivi-dom/src/ffi.rs`（修改：`build_naive_namespace` 改 `flush_frame`/`set_frame_rejected_callback`；`drain_events` 改事件帧）
  - `packages/naivi-native/src/main.rs`（修改：`QuickJsEventSink` 改事件帧编码；tick 调 guest `__tick`）
...
- **Approach:**
  ...
  2. `QuickJsEventSink` 用 U6 事件帧编码；`drain_events` 把事件帧字节交给 JS 回调。
...
- **Test scenarios:**
  ...
  - 事件帧：合成 pointer/keyboard 事件 → JS 收到虚拟 id + key/code/value。
...
- **Global:**
  ...
  - 事件 kind / op / payload 协议无任何手写同步点（除 R5 的 blitz 映射边界）；加一种事件 kind 只改 SOT 一处。
